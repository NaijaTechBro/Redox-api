...
const createSuperAdmin = async (req, res) => {
  ...
  try {
        ...
        if (!name || !email || !password) {
          res.status(400);
          throw new Error("Please provide name, password, and email !");
        }
...
const sendLoginCode = asyncHandler(async (req, res) => {
  ...
  const subject = "Login Access Code - Redox";
  const send_to = email;
  const sent_from = "Nestlypay <hello@seemetracker.com>";
...
const sendVerificationEmail = asyncHandler(async (req, res) => {
  ...
  const verificationUrl = `${process.env.REDOX_FRONT_END_LIVE_UR}/admin/auth/email/verify/?verification_token=${verificationToken}`;
